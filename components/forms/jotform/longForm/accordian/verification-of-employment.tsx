...
export function VerificationOfEmployment({
  form,
  hideSSNInput = false,
  hideSignature = false,
  ...
  const handleSignatureChange = (signature: string | null) => {
    const signatureEntity = new ApplicantExtrasEntity(ApplicantExtras.SIGNATURE_VOE_AUTHORIZATION);
  ...
  useEffect(() => {
    ...
    if (form.values.ssn && form.values.ssn !== applicant.ssn && form.values.ssn.length === 9) {
          ...
          {!!current_company?.is_current && (
            <>
              ...
              {current_company?.manager_name && (
                <p className={`${styles.paragraph} ${styles.align__text_left} w-75`}>
                  {t(
                  ...
                  <>
                    ...
                    <p className={`${styles.paragraph} ${styles.align__text_left} w-75`}>
                      {t(
                        'DESIGNATED_EMPLOYER_REPRESENTATIVE_{current_manager_name}',
                        {
                          current_manager_name: v?.manager_name ? v?.manager_name : `${t('N/A')}`,
                        },
                        { translateProps: true }
                      )}
                    </p>

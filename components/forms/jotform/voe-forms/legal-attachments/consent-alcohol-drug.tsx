...
export default function ConsentAlcoholDrug({ applicant }: ConsentAlcoholDrugProps) {
  ...
  const signature = applicant?.extras?.find(
    (sign) => sign?.type == ApplicantExtras.SIGNATURE_GENERAL_CONSENT
  );
  const generalConsent = applicant?.extras?.find(
    (consent) => consent?.type == ApplicantExtras.GENERAL_CONSENT
  );
  const date = applicant?.extras?.find((v) => v.type == ApplicantExtras.APPLY_DATE);
  ...
  return (
    <form>
      ...
      <div className="Row" style={{ textAlign: 'left', marginBottom: '20px' }}>
        <div className="Col">
          <p style={{ color: 'black', fontWeight: 'bold', display: 'inline' }}>
            {t('EMPLOYERS_NAME:')}
          </p>
      ...
      <div className="Row">
        <p style={{ color: 'black', display: 'inline' }}>{t('INSSTRUCTIONS_SECTION')}</p>
      </div>
    </form>
  );
}

    ...
    if (user && user.id) {
      try {
        if (!user.company) {
          ...
          if (data) {
            const preferences = await userApi.preferences.list(user.id, {
            ...
            if (preferences?.length > 0) {
              data.documents = applicant.documents.filter(
                (document) =>
                  !preferences.some(
                    (preference) =>
                      preference.label == document.type && preference.value == SharePreference.NEVER
                  )
              );
            } else
              data.documents = data.documents?.filter(
                (document) => document.type == ApplicantDocumentType.RESUME_OPTIONAL
              );
          }
          setApplicant(data);
        }
    ...
    <>
      {applicant?.jobs?.length > 0 && applicant?.jobs?.some((item) => item?.job?.id == job?.id) ? (
        <div className="ort-btn mt-lg-4 mt-0">
          <button type="button" className="btn theme-primary-btn" disabled={true}>
            {' '}
            {t('APPLIED')}
          </button>
        </div>
      ...
      <ViewModal
        ...
        footer={
          showForm && (
            <button
              disabled={
                !!apply_form.isSubmitting || !!!apply_form.isValid || !!apply_form.isValidating
              }
              type="submit"
          ...
          <form onSubmit={apply_form.handleSubmit}>
            ...
            <Row>
              <Col sm="12" className="mt-3">
                  ...
                  actions={
                    <Button
                      ...
                      onClick={() =>
                        apply_form.setValues({
                          ...apply_form.values,
                          documents: [...(apply_form.values.documents || []), new DocumentEntity()],
                        })
                      }
                    >
                      <PlusCircle /> {t('ADD')}
                ...
                >
                  {!apply_form.values.documents?.length && t('NONE')}
                  {apply_form.values.documents?.length > 0 && (
                    <>
                      <Row className="p-1">
                        {apply_form.values.documents.map((entity, i) => (
                            ...
                            <div className="col-md-1 mt-lg-3 mt-md-3 mt-1 text-right p-0">
                              <button
                                onClick={() =>
                                  apply_form.setValues({
                                    ...apply_form.values,
                                    documents: apply_form.values.documents.filter(
                                      (v, idx) => i != idx
                                    ),
                                  })
                                }
            ...
            <Row>
              <div className="font-weight-bold col-12 p-0">
                <p className="mx-3 mt-5">
                  {t('i_accept_{terms_and_condition}', {
                    terms_and_condition: t('terms_and_condition'),
                  })}
                </p>
              </div>
            </Row>
          ...
          <Row>
            {Boolean(showDrugErrorMessage) ? (
              <Row>
                ...
                <h6 className={`${styles.paragraph} ${styles.margin__top} p-1`}>
                  {t('DRUG_TEST_VALIDATION_FAIL_MESSAGE_REVIEW_SAP')}{' '}
                  <Link href={'https://www.transportation.gov/odapc/employee#SAP'}>
                    {t('SAP_RESOURCE_PAGE')}
                  </Link>
                  .
                </h6>
                <h6 className={`${styles.paragraph} ${styles.margin__top} p-1`}>
                  {t('DRUG_TEST_VALIDATION_FAIL_MESSAGE_ENCOURAGE_REVIEW')}{' '}
                  <Link href={' https://www.naadac.org/sap-directory?'}>{t('HERE')}</Link>
                </h6>
                <h6 className={`${styles.paragraph} ${styles.margin__top} p-1`}>
              ...
              <p>
                {!user?.id ? (
                  <>
                    {t('QUICK_APPLY_MESSAGE_{JOB}_{COMPANY}', {
                      JOB: job?.title,
                      COMPANY: job?.company?.name,
                    })}
                    <Link href={`/apply/quick-apply/${applicant?.uuid_token}`}>

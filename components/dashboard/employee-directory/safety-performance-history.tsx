...
export default function SafetyPerformanceHistory({
  ...
  const form = useFormik({
    ...
    onSubmit: async ({ document, employer }, { resetForm }) => {
      try {
        const doc = await employeeApi.employer.documents.create(
          employee.id,
          employer.id,
          document
        );
  ...
  const handleDeleteDocument = async (
    ...
    await employeeApi.employer.documents.delete(
      employee?.id,
      employer?.id,
      docType
    );
  ...
  const resendVoeRequest = async (employerId: number) => {
    try {
      const response: EmployeeEmployerEntity =
        await employeeApi.employer.sendVoeRequest(employee?.id, employerId);
      setEmployers([
            ...
            },
            {
              ...
              cell: (emp) => {
                const doc = emp.documents?.find(
                  (v) => v.type == EmployeeDqf.SAFETY_PERFORMANCE_HISTORY
                );
                return (
                  <>
                    <ButtonList

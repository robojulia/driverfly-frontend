...
export default function ViewDocumentHistory({
  ...
  const viewHistory = async () => {
    const data = await documentApi.getDocumentHistory({
      type: type,
      documentable_type:
        (doc?.documentable_type as string) ?? documentable_type,
  ...
  const handleViewDocument = async (
    ...
    const document: DocumentHistoryEntity =
      await documentApi.getSignedUrlForHistory(id);
  ...
  const handleDownloadDocument = async (id: number): Promise<void> => {
    const doc: DocumentHistoryEntity = await documentApi.getSignedUrlForHistory(
      id
    );
          ...
          columns={[
            {
              name: "TYPE",
              selector: (doc) =>
                typePrefix ? t(`${typePrefix}.${doc.type}`) : doc.type,
              hidable: false,
            },
            {
              name: "NAME",

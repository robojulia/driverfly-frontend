...
import { ArrowUp, ArrowDown, Tools } from 'react-bootstrap-icons';
...
export interface MaintenanceReportWithVehicle extends VehicleMaintenanceReportEntity {
  vehicle?: VehicleEntity;
}

interface AllMaintenanceReportsTableProps {
  maintenanceReports: MaintenanceReportWithVehicle[];
  loading?: boolean;
}

export const AllMaintenanceReportsTable: React.FC<AllMaintenanceReportsTableProps> = ({
  maintenanceReports,
  loading = false,
}) => {
  const { t } = useTranslation();
  const router = useRouter();
  const [sortField, setSortField] = useState<string | null>('maintenance_date');
  ...
  const handleRowClick = (report: MaintenanceReportWithVehicle) => {
    if (report.vehicle) {
      router.push(`/dashboard/company/settings/vehicles/${report.vehicle.id}`);
    }
  };
  ...
  if (loading) {
    return (
      <div className="text-center py-5">
        ...
        <p className="mt-3 text-muted">{t('Loading maintenance reports...')}</p>
      </div>
    );
  }
      ...
      {maintenanceReports.length === 0 ? (
        <div className="text-center py-5 bg-light rounded">
          ...
          <p className="text-muted mb-3">
            {t('No maintenance reports have been uploaded yet.')}
          </p>
        </div>
      ) : (
        <>
          <Table striped bordered hover className={`custom-table ${styles.repairsTable}`}>
            ...
            <tbody>
              {getPaginatedReports().map((report) => (
                <tr
                  key={`${report.vehicle?.id}-${report.id}`}
                ...
                >
                    ...
                    {report.vehicle ? (
                      <div>
                        <div className="fw-bold">
                          {report.vehicle.unit_number
                            ? `Unit #${report.vehicle.unit_number}`
                            : t('No Unit')}
                        </div>
                  ...
                  <td>
                    <div className="text-truncate" style={{ maxWidth: '250px' }} title={report.description || ''}>
                      {report.description || <span className="text-muted">{t('No description')}</span>}
                    </div>
                  </td>
                  <td>
